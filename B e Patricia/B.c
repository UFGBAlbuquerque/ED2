...
void suggestWords(BTreeNode *root, const char *key){
    if (root == NULL) {
        return;
    }
    ...
    if (root->isLeaf) {
        for (int i = 0; i < root->numKeys; i++) {
            if (strncmp(root->keys[i], key, strlen(key)) == 0) {
                printf("%s\n", root->keys[i]);
            }
        }
    } else {
        ...
        for (int j = i; j < root->numKeys; j++) {
            suggestWords(root->children[j], key);

            if (strncmp(root->keys[j], key, strlen(key)) == 0) {
                printf("%s\n", root->keys[j]);
            }
        }

        suggestWords(root->children[root->numKeys], key);
    }
}
...
int main() {
    ...
    insertBTree(&root, "canto");
    insertBTree(&root, "co");
    insertBTree(&root, "doggy");
    insertBTree(&root, "dart");
    insertBTree(&root, "drat");
    insertBTree(&root, "B");
    ...
    printf("\npalavras sugeridas com 'ca': \n");
    suggestWords(root, "ca");
